...
<ul>
<li>a) tutela della salute, prevenzione, diagnosi, cura, riabilitazione, assistenza;</li>
<li>b) attività amministrative connesse alle prestazioni;</li>
<li>c) igiene e sicurezza nei luoghi di lavoro;</li>
<li>d) gestione e valutazione delle sedute chinesiologiche;</li>
<li>e) obblighi di legge;</li>
<li>f) difesa in giudizio;</li>
<li>g) ricerca scientifica e ricontatto del paziente per finalità affini;</li>
<li>h) comunicazione dello stato di salute a familiari/terzi;</li>
<li>i) comunicazione della presenza presso lo Studio;</li>
<li>j) comunicazione a compagnie assicurative.</li>
</ul>
...
<p><strong>Luogo/Data</strong></p>
<p><strong>Firma dell’interessato (o genitore/tutore in caso di minore)</strong></p>
<p><strong>Firma del Titolare del trattamento che ha reso l’informativa</strong></p>
"""
}
...
<form method="get" action="/form">
    ...
    <strong>Dichiaro inoltre di:<p>
        <label><input type="radio" name="consenso1" value="acconsento" required> Prestare il mio consenso per le finalità di cui al punto 2, lett. C) </label><br>
...
<div style="display: inline-block;">
  ...
  <div style="position: relative; border: 1px solid #000; width: 500px; height: 150px;">
    ...
    <canvas id="signature-pad-serioli-2" width="400" height="200" style="border:1px solid #000;"></canvas>
    ...
    <button type="button" onclick="clearSignature('serioli')" 
            style="position: absolute; top: -40px; right: -15px; background: none; border: none; color: red; font-weight: bold;">X</button>
...
<form method="post" action="/submit_taesi">
  <input type="hidden" name="nome_cognome" value="{{ nome }}">

  <label for="luogo_taesi"><strong>Luogo:</strong></label>
  <input type="text" name="luogo_taesi" required><br>

  <label for="data_taesi"><strong>Data:</strong></label>
  <input type="text" name="data_taesi" placeholder="gg/mm/aaaa" required><br><br>
  ...
  <button type="button" onclick="clearSignature('taesi')" style="color:red;">X</button>
  ...
  <button type="button" onclick="clearSignature('taesi2')" style="color:red;">X</button>
...
<script>
  const canvas1 = document.getElementById("signature-pad-taesi");
  const input1 = document.getElementById("signature_data_taesi");
  const canvas2 = document.getElementById("signature-pad-taesi-2");
  const input2 = document.getElementById("signature_data_taesi_2");
  const form = document.querySelector('form[action="/submit_taesi"]');
  const pad1 = canvas1 ? new SignaturePad(canvas1) : null;
  const pad2 = canvas2 ? new SignaturePad(canvas2) : null;

  window.clearSignature = function(tipo) {
    if (tipo === 'taesi' && pad1) pad1.clear();
    if (tipo === 'taesi2' && pad2) pad2.clear();
  };

  if (form) {
    form.addEventListener("submit", function(e) {
      if (!pad1 || pad1.isEmpty()) {
        alert("Per favore firma nel primo riquadro prima di inviare.");
        e.preventDefault();
        return;
      }
      input1.value = pad1.toDataURL();
      if (pad2 && !pad2.isEmpty()) {
        input2.value = pad2.toDataURL();
      }
    });
  }
</script>
...
<!-- Firma Serioli: Script SignaturePad -->
<script src="https://cdn.jsdelivr.net/npm/signature_pad@4.0.0/dist/signature_pad.umd.min.js"></script>
<script>
  document.addEventListener("DOMContentLoaded", function () {
    const canvas1 = document.getElementById("signature-pad-serioli");
    const input1 = document.getElementById("signature_data_serioli");
    const canvas2 = document.getElementById("signature-pad-serioli-2");
    const input2 = document.getElementById("signature_data_serioli_2");
    const form = document.querySelector('form[action="/submit_serioli"]');

    const pad1 = canvas1 ? new SignaturePad(canvas1) : null;
    const pad2 = canvas2 ? new SignaturePad(canvas2) : null;

    // funzione per cancellare le firme
    window.clearSignature = function(tipo) {
      if (tipo === 'serioli' && pad1) pad1.clear();
      if (tipo === 'serioli2' && pad2) pad2.clear();
    };
    ...
    if (form && pad1) {
      form.addEventListener("submit", function (e) {
        ...
        input1.value = pad1.toDataURL();  // salva prima firma

        if (pad2 && !pad2.isEmpty()) {
          input2.value = pad2.toDataURL();  // salva seconda firma (se presente)
        }
      });
    }
  });
...
<script>
document.addEventListener("DOMContentLoaded", function() {
  const canvasSerioli = document.getElementById('signature-pad-serioli');
  const inputSerioli = document.getElementById('signature_data_serioli');
  const formSerioli = document.querySelector('form[action="/submit_serioli"]');
  const canvas2 = document.getElementById("signature-pad-serioli-2");
  const input2 = document.getElementById("signature_data_serioli_2");
  let padSerioli = null;

  if (canvasSerioli && inputSerioli && formSerioli) {
    padSerioli = new SignaturePad(canvasSerioli);

    formSerioli.addEventListener('submit', function(e) {
      if (padSerioli.isEmpty()) {
        alert("Per favore firma prima di inviare.");
        e.preventDefault();
      } else {
        const dataURL = padSerioli.toDataURL();
        inputSerioli.value = dataURL;
        console.log("Firma salvata:", dataURL);  // DEBUG
      }
    if (canvas2 && input2) {
...
<form method="post" action="/submit_serioli">
  ...
  <div class="canvas-container">
    ...
    <button type="button" class="clear-btn" onclick="clearSignature('serioli')">X</button>
  </div>
...
<script>
    ...
    const padSerioli = canvasSerioli ? new SignaturePad(canvasSerioli) : null;
  ...
  function clearSignature(tipo) {
    if (tipo === 'scattolini' && padScattolini) {
      ...
      padGenerale.clear();
    }  else if (tipo === 'serioli' && padSerioli) {
    padSerioli.clear();
  }
  }
...
if (formSerioli) {
  formSerioli.addEventListener('submit', function(e) {
    if (padSerioli && padSerioli.isEmpty()) {
      alert("Per favore firma prima di inviare.");
      e.preventDefault();
    } else {
      inputSerioli.value = padSerioli.toDataURL();
    }
  });
}
...
<div class="firma-box">
    ...
    <button type="button" class="clear-btn" onclick="clearCanvas()">X</button>
</div>
...
def submit_taesi():
    ...
    data = request.form.get("data_taesi", "").strip()
