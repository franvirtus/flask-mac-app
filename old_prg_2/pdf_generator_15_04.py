...
def generate_pdf(data, specialist, specialist_id, output_dir="output"):
    ...
    # Testo informativa
    pdf.set_font("Arial", size=12)
    pdf.write_html_formatted(specialist["testo_informativa"])
    pdf.ln(5)
    ...
    if specialist.get("dichiarazione_finale_blocco", {}).get("attivo"):
        template = specialist["dichiarazione_finale_blocco"]["template"]
    ...
    for i in range(specialist.get("firme", 0)):
        ...
        if key in data and data[key]:
            try:
                ...
                pdf.image(img_stream, x=pdf.get_x(), y=pdf.get_y(), w=60)
                pdf.ln(30)
            except Exception as e:
                pdf.cell(0, 10, f"Errore nella firma {i + 1}", ln=True)
