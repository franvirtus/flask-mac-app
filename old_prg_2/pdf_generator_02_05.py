...
# SOLO per Morelli: inseriamo testo informativa e dichiarazioni
    ...
    # Dati paziente
    campi_aggiuntivi = specialist.get("campi_aggiuntivi", [])
    if campi_aggiuntivi and specialist.get("nome") != "Dott.ssa Veronica Lorenzini":
        pdf.set_font("DejaVu", style="B", size=12)
        pdf.cell(0, 10, "Dati del paziente:", ln=True)
        pdf.set_font("DejaVu", size=12)
        for campo in campi_aggiuntivi:
            valore = sanitize_text(data.get(campo, ""))
            pdf.cell(0, 10, f"{campo.capitalize()}: {valore}", ln=True)
    pdf.ln(5)
    ...
    if specialist.get("nome") == "Dott.ssa Veronica Lorenzini":
        ...
        testo_finale = (
            f"Io sottoscritto/a {nome} {cognome}, nato/a il {data_nascita}, residente a {indirizzo} acquisite le summenzionate informazioni fornite dal Titolare del trattamento ai sensi dell’art. 13 del Reg. UE, e consapevole, in particolare, che il trattamento potrà riguardare dati relativi alla salute, presto il mio consenso per il trattamento dei suddetti dati per le finalità di cui al punto 1, lett. A) dell'informativa."
        )
        pdf.ln(10)
        pdf.set_font("DejaVu", "", 12)
