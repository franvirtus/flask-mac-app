...
<script>
  ...
  // Salva la firma nel campo hidden prima del submit
  document.querySelectorAll('form').forEach(function(form) {
    form.addEventListener('submit', function(e) {
      if (padSerioli) {
        if (padSerioli.isEmpty()) {
          alert("Per favore firma prima di inviare.");
          e.preventDefault();  // blocca il form
        } else {
          inputSerioli.value = padSerioli.toDataURL();  // salva firma
        }
      }
    });
  });
...
def submit_serioli():
    ...
    firma_data = request.form.get("signature_data_serioli", "")
