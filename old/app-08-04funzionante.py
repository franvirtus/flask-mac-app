...
def submit_serioli():
    signature_data = request.form.get("signature_data_serioli")
    ...
    tutore = request.form.get('tutore', '').strip()
    consenso1 = request.form.get('consenso1', 'Non indicato')
    consenso2 = request.form.get('consenso2', 'Non indicato')
    consenso3 = request.form.get('consenso3', 'Non indicato')
    data_serioli = request.form.get('data', datetime.datetime.now().strftime('%d/%m/%Y'))
    firma_data = signature_data

    if not firma_data.startswith("data:image/png;base64"):
        return "<h1>Errore: firma mancante o non valida</h1>"

    firma_img = f'<img src="{firma_data}" alt="Firma" style="width:200px;"/>'

    html = f"""
    <!DOCTYPE html>
    <html>
    <head><meta charset="utf-8"><title>Consenso Dott.ssa Serioli</title></head>
    <body style="font-family: Arial, sans-serif; line-height: 1.6;">
      <h2>Modulo Consenso - Dott.ssa Sara Serioli</h2>
      ...
      <hr>
      <p><strong>Firma:</strong><br>{firma_img}</p>
    </body>
    </html>
    """

    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    folder = os.path.join(SHARED_FOLDER, "Dott.ssa Serioli")
    os.makedirs(folder, exist_ok=True)
    filename = f"serioli_consenso_{timestamp}.pdf"
    ...
    return f"""
    <h1>PDF generato con successo!</h1>
    <p>File salvato come: <strong>{filename}</strong></p>
    <p><a href='/'>Torna alla Home</a></p>
    """
