...
def home():
    ...
    for key, info in specialists_data.items():
        # Ora info è un dizionario, es. {"nome": "Dott_Rossi", "modulo": "..."}
        name = info["nome"]  # "Dott_Rossi"
        specialists_buttons += f'<button onclick="window.location.href=\'/form?specialist={key}\'">{name}</button>\n'
    ...

# ==========================
# Carica il logo in base64 (usato solo nella homepage)
# ==========================
logo_path = "logo.png"
...
def home():
    ...
    specialists_buttons = ""
    for key, info in specialists_data.items():
        # Usa il nome da info["nome"] per il bottone
        specialists_buttons += f'<button onclick="window.location.href=\'/privacy/{key}\'">{info["nome"]}</button>\n'
...
def submit():
    ...
    # Usa il nome definito nel file specialists.json come cartella (es. "Dott_Rossi")
